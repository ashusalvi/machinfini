...
<div class="lecture-sidebar">
    ...
    @if($auth_user)
        ...
        <div class="mb-4 text-center"> <a href="https://www.machinfini.com/plugin/certificate_seminar/{{$course->id}}/download" class="btn btn-success"> <i class="la la-certificate"></i> Download Certificate</a> </div>
                            ...
                            @foreach($section->items as $item)
                                ...
                                <div class="sidebar-section-item {{$item->id == $content->id? 'active' : ''}} {{$is_completed? 'completed' : ''}}">
                                    <div class="section-item-title border-bottom">
                                        ...
                                        <a href="{{route('seminar_single_'.$item->item_type, [$course->slug, $item->id ] )}}" class="p-2 d-flex" @if($is_completed) data-toggle="tooltip" title="{{__t('completed')}}" @endif>
