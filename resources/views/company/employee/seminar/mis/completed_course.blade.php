            ...
            <table id="earning_table" class="table table-striped table-bordered display">
                ...
                <tbody>
                    @foreach ($completed_courses as $completed_course)
                        ...
                        @endphp
                        <tr>
                            <td><a href="{{route('course', $completed_course->course->slug)}}" target="_blank">{{ $completed_course->course->title }}</a></td>
                            <td>{{ $completed_course->companyEmployee($completed_course->user_id)->employee_id }}</td>
                            <td>{{ $completed_course->companyEmployee($completed_course->user_id)->user->name }}</td>
                            <td>{{ $completed_course->companyEmployee($completed_course->user_id)->user->email }}</td>
                            <td>{{ $completed_course->companyEmployee($completed_course->user_id)->user->phone }}</td>
                            ...
                            <?php 
                                if(getScore($completed_course->course->id,Auth::user()->id) != '-1'){
                            ?>
                            <td>{{ getScore($completed_course->course->id,Auth::user()->id) }} / {{ TotalScore($completed_course->course->id,Auth::user()->id) }}</td>
                            ...
                            <?php 
                                if(getScore($completed_course->course->id,Auth::user()->id) != '-1'){
                            ?>
                            <td>{{ round((getScore($completed_course->course->id,Auth::user()->id) / TotalScore($completed_course->course->id,Auth::user()->id) ) * 100,2) }}% </td>
                            ...
                            ?>
                                <td>
                                    <?php
                                        $percentage_value = (getScore($completed_course->course->id,Auth::user()->id) / TotalScore($completed_course->course->id,Auth::user()->id) ) * 100;
...
<script>
    $(function() {
        $('input[name="daterange"]').daterangepicker({
            ...
            let url = '/company/employee/all_employee_completed_course/'+start_date+'/'+end_date;
