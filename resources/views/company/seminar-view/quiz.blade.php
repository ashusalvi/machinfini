@extends('company.template-part.layout-full')

@section('content')

@include('company.template-part.content-navigation', ['content' => $quiz, 'quiz' => 1])
<div class="lecture-container-wrap d-flex">
    @include('company.template-part.curriculum_item_sidebar', ['content' => $quiz])
    <div class="lecture-container">
        ...
        <div class="quiz-instruction-wrapper">
            ...
            @php
            $attempt = $auth_user->get_seminar_attempt($quiz->id);
            $passing_percent = 0;
            @endphp
            {{-- @if (!empty($attempt))
            ...
            @if(! $attempt || $attempt->status !== 'finished' ) <div class="assignment-header-info mb-4 p-3">
                <h4 class="mb-4">Instructions</h4>

                <p class="assignment-basic-info mb-2">
                    <span class="mr-4"> <i class="la la-clock"></i> <strong> Duration : </strong>
                        {{$quiz->option('time_limit')}} Minutes </span>

                    <span class="mr-4"><i class="la la-chart-pie"></i> <strong> Total Questions : </strong>
                        {{$quiz->option('questions_limit')}}</span>
                    <span class="mr-4"><i class="la la-check-circle"></i> <strong> Minimum Pass Score : </strong>
                        {{$quiz->option('passing_score')}}%</span>
                </p>

                <p class="font-italic text-muted m-0">
                    Must be finished in one sitting. You cannot save and finish later. Will not let you finish with any
            ...
            @if($attempt)
                ...
                @if($attempt->status === 'in_review')
                    <div class="bg-light-sky p-4 border d-flex quiz-submitted-alert">
                        ...
                        <div>
                            ...
                            <p>You've submitted this quiz and result is in review, and instructor will review your result soon,
                                after that, you can see your result.</p>
                        </div>
                    </div>
                @endif
                ...
                @if($attempt->status === 'finished')
                    ...
                    @if ($attempt->passed == 0)
                    <div id="start-quiz-btn-wrapper" class="mt-5 mb-3">
                        <a href="{{ route('seminar_quiz_re-quiz',[$quiz->id]) }}"><button id="btn-re-start-quiz" class="btn btn-success btn-lg"
                                data-quiz-id="{{$quiz->id}}">
                                <i class="la la-play-circle"></i>Re-Start Quiz
                            </button></a>
                    </div>
                    @endif
                    ...
                    <div class="quiz-attempt-stats mb-3">
                        ...
                        <ul class="list-group list-group-horizontal">
                            ...
                            <li class="list-group-item">
                                ...
                                @else
                                <p class="mb-1 text-danger"> <strong> <i class="la la-exclamation-circle"></i> Failed</strong>
                                </p>
                                @endif
                            </li>
                            <li class="list-group-item">
                    ...
                    <div class="quiz-result-gretings">
                        ...
                        <div class="bg-success text-white p-3 border text-center">
                            ...
                            <h3 class="mb-3">You passed with <strong>{{$attempt->earned_scores}} ({{$attempt->earned_percent}}%)</strong> Score
                            </h3>
                        </div>
                        @else
                        <div class="bg-warning p-3 border text-center">
                            ...
                            <h2>RETRY</h2>
                            <h3 class="mb-3">You have not cleared, kindly go through the knowledge sessions again and RETRY.</h3>
                        </div>
                        @endif
                    </div>
                            ...
                            <div class="col-md-6">
                                <div class="view-attempted-answer border p-3 mb-3">
                                    ...
                                    <p class="text-success mb-0">
                                        {{-- <span class="badge badge-info mr-3"> <i class="la la-question-{{$qtype}}"></i>
                                            {{$qtype}}</span> --}}
        ...
        @else
            <div class="lecture-contents-locked text-center mt-5">
                ...
                <p class="lecture-lock-desc mb-4">
                    {!! sprintf(__t('if_enrolled_login_text'), '<a href="'.route('login').'" class="open_login_modal">',
                        '</a>')
                    !!}
                </p>
                @endif
                <a href="{{route('course', $course->slug)}}" class="btn btn-theme-primary btn-lg">Enroll in Course to
                    Unlock</a>
            </div>
        @endif
    </div>
