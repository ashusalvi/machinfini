...
class User extends Authenticatable
{
    ...
    {
        return $this->belongsTo(collage::class, 'college_id');
    }

    public function channelPartner()
    {
        return $this->hasOne(channelPartner::class, 'user_id','id');
    }

    public function referancechannelPartner()
    {
        return $this->hasOne(channelPartner::class, 'user_id','apply_reference_code');
    }
...
where course_user.user_id = {$this->id} and reviews.status = 1";
    ...
    public function isChannelPartner(){
        return Auth::user()->user_type === 'channel_partner';
    }
    ...
    public function isCollageInstructor(){
        return $this->college_user_type  == '2';
    }
    ...
    public function isSeminarEnrolled($course_id = 0){
        if ($course_id === 0){
            return false;
        }

        $isEnrolled = DB::table('company_seminar_enrolls')->whereUserId($this->id)->whereCourseId($course_id)->whereStatus('success')->orderBy('enrolled_at', 'desc')->first();
    ...
    public function complete_seminar($course_id = null){
        if ( ! $course_id){
            return false;
        }

        $is_completed = CompanySeminarComplete::wherecompletedSeminarId($course_id)->whereUserId($this->id)->first();
        ...
        $data = [
            'user_id'               => $this->id,
            'completed_seminar_id'   => $course_id,
            'completed_at'          => Carbon::now()->toDateTimeString(),
        ];
        return CompanySeminarComplete::create($data);
    }
    ...
    public function is_completed_seminar($course_id){
        $is_completed = CompanySeminarComplete::wherecompletedSeminarId($course_id)->whereUserId($this->id)->first();
        return $is_completed;
    }
    ...
    public function get_seminar_attempt($quiz_id){
        $attempt = CompanyAttempt::where('user_id', $this->id)->where('quiz_id', $quiz_id)->first();
        return $attempt;
    }
