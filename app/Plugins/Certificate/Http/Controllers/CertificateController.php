...
class CertificateController extends Controller{
    ...
    public function __construct(){
        ...
        // root_path('Plugins/CertificateAssets/layouts/');
        // $this->layouts_path = define("ROOT_PATH", "Plugins/CertificateAssets/layouts/");
    ...
    public function generateSeminarCertificate($course_id){
        if ( ! Auth::check()){
            abort(404);
        }
        ...
        $isCourseComplete = $this->user->is_completed_seminar($course_id);
        if ( ! $isCourseComplete){
            abort(404);
        }
        $this->course = CompanySeminar::find($course_id);
        if ( ! $this->course){
        ...
        $dompdf->loadHtml($this->seminarContent());
        $dompdf->setPaper('A4', 'landscape');
        $dompdf->render();
    ...
    public function seminarContent(){
        ...
        ob_start();
        include $certificate_path.'/certificate_seminar.php';
        $content = ob_get_clean();
