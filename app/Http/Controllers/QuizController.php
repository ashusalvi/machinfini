...
class QuizController extends Controller
{
    ...
    public function quizView($slug, $quiz_id){
        ...
        if (Auth::check()){
            ...
            // $attempt = $user->get_attempt($quiz->id);

            // if($attempt != null){
            //     if($attempt->passed == 0){
            //         $delete_complete_flag = Complete::where('content_id',$quiz_id)->delete();
            //     }
            // }
        }
    ...
    public function quizAttempting($quiz_id){
        ...
        if ( ! $attempt || $attempt->status !== 'started' || $attempt->passed == 1) {
            abort(404);
        }
        ...
        if ($answered->count() >= $question_limits){
            ...
            $reviewRequired = Answer::query()->where('quiz_id',$quiz_id)->where(function($q){
    ...
    public function reattendQuiz(Request $request,$quiz_id){
        ...
        $attempt = Attempt::where('quiz_id',$quiz_id)->where('user_id',Auth::user()->id)->delete();

        $answer = Answer::where('quiz_id',$quiz_id)->where('user_id',Auth::user()->id)->delete();
