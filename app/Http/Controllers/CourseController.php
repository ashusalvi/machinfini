...
class CourseController extends Controller
{
    ...
    public function view(Request $request,$slug,$id = NULL,$cp_id = NULL,$session = NULL){
        session(['cp_id_link' => $cp_id,'session_link' => $session,'affilite_id'=>$id]);

        if ($cp_id != NULL) {
            // $decrypted_code = decrypt($code);
            $user_code = User::where('id',$cp_id)->with('channelPartner')->first();
            
            if (!empty($user_code) > 0)
            {   
                $partner_logo = $user_code->channelPartner->logo;
                session(['ref_code' => $cp_id]);
        ...
        $user = Auth::user();
        if ( $course->status != 1 && $user->user_type != 'admin')
        {
        ...
        session(['cp_slug' => $course_slug]);
        session(['cp_course_id' => $course->id]);
        $is_open = 0;

        if(isset($user->company_id) && $user->company_id != 0)
        {   
            $user_id        = Auth::user()->id;
            $employee_details = CompanyEmployee::where('user_id',$user_id)->first();
            if(!empty($employee_details)){
                $Coupon = Coupon::where('company_id',$user->company_id)
        ...
        if (Auth::check())
        {
            ...
            if($user->user_type == 'admin' || $course->user_id == $user->id){
                $is_open = 1;
            }
        ...
        return view(theme('course'), compact('course', 'title', 'isEnrolled','is_open'));
    }
    ...
    public function create(){
        ...
        if(Auth::user()->user_type == 'admin'){
            $teachers = User::where('user_type','instructor')->get();
            $is_admin = 1;
        }

        return view(theme('dashboard.courses.create_course'), compact('title', 'categories','is_admin','teachers'));
    }
    ...
    public function store(Request $request){
        ...
        if ( isset($request->teacher_name) && $request->teacher_name != 0 ) {
    ...
    public function information($course_id){
        ...
        if ( ! $course || ( ! $course->i_am_instructor  && ! Auth::user()->user_type == 'admin') ){
            abort(404);
        }
        $categories = Category::parent()->get();
    ...
    public function myCourses(){
        ...
        $course_by_admin = null;
        // for admin 
        if(Auth::user()->user_type == 'admin'){
            $course_by_admin = course::where('create_by_admin',1)->get();
        }

        return view(theme('dashboard.my_courses'), compact('title','course_by_admin'));
    }
