...
function get_certificate($id = null){

    if($id != null){
        $course = Course::where('id',$id)->first();
        return media_image_uri($course->author->signature);
    }else{
        return 'Invalid data';
    }
...
function get_certificate_course($id = null){
...
function get_certificate_seminar($id = null){

    if($id != null){
        $course = CompanySeminar::where('id',$id)->first();
        return $course->title;
    }else{
        return 'Invalid data';
    }
...
function getScore($id = null,$user_id = null){
    if($id != null && $user_id != null){
        $attemps_score_status = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->count();
        if($attemps_score_status != 0){
            return $attemps_score = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->sum('earned_scores');
        }else{
            return '-1';
        }
    }else{
...
function TotalScore($id = null, $user_id = null){
    if($id != null && $user_id != null){
        $attemps_score_status = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->count();
        if($attemps_score_status != 0){
            return $attemps_score = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->sum('total_scores');
        }else{
            return '-1';
        }
    }else{
...
function getCompanyScore($id = null,$user_id = null){
    if($id != null && $user_id != null){
        $attemps_score_status = CompanyAttempt::where('seminar_id',$id)->where('user_id', $user_id)->where('passed',1)->count();
        if($attemps_score_status != 0){
            return $attemps_score = CompanyAttempt::where('seminar_id',$id)->where('user_id', $user_id)->where('passed',1)->sum('earned_scores');
        }else{
            return '-1';
        }
    }else{
...
function TotalCompanyScore($id = null, $user_id = null){
    if($id != null && $user_id != null){
        $attemps_score_status = CompanyAttempt::where('seminar_id',$id)->where('user_id', $user_id)->where('passed',1)->count();
        if($attemps_score_status != 0){
            return $attemps_score = CompanyAttempt::where('seminar_id',$id)->where('user_id', $user_id)->where('passed',1)->sum('total_scores');
        }else{
            return '-1';
        }
    }else{
...
function getPercentage($id = null,$user_id = null){
    if($id != null && $user_id != null){
        $attemps_percentage_status = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->count();
        if($attemps_percentage_status != 0){
            return $attemps_percentage = Attempt::where('course_id',$id)->where('user_id', $user_id)->where('passed',1)->sum('earned_percent');
        }else{
            return '-1';
        }
    }else{
...
function get_complete_date($course_id=null, $user_id=null){
    $getcomplete = Complete::where('user_id',$user_id)->where('completed_course_id',$course_id)->first();
    return $getcomplete->completed_at;
}

function get_seminar_complete_date($course_id=null, $user_id=null){
    $getcomplete = CompanySeminarComplete::where('user_id',$user_id)->where('completed_seminar_id',$course_id)->first();
    return $getcomplete->completed_at;
}
...
if ( ! function_exists('view_seminar_template_part')) {
    function view_seminar_template_part($view = null, $data = [], $mergeData = []){
...
if ( ! function_exists('complete_content')) {
    function complete_content($content, $user){
        ...
        $is_true =1 ;
        if($content->item_type == 'quiz'){
            $attempt = $user->get_attempt($content->id);
...
if ( ! function_exists('complete_seminar_content')) {
    function complete_seminar_content($content, $user){
        if (!$content || !$user) {
            return false;
        }

        if (!$content instanceof Content) {
            $content = CompanySeminarContent::find($content);
        }
        if (!$user instanceof \App\User) {
            $user = \App\User::find($user);
        }
        ...
        if($is_true){
            $is_completed = CompanySeminarComplete::whereContentId($content->id)->whereUserId($user->id)->first();
            ...
            if ( ! $is_completed) {
                ...
                $complete = CompanySeminarComplete::create($data);

                do_action('complete_course', $complete);
            }

            $total_contents = (int) CompanySeminarContent::whereSeminarId($seminar_id)->count();
            $completes = CompanySeminarComplete::whereUserId($user->id)->whereSeminarId($seminar_id)->pluck('content_id');
            $completed_count = $completes->count();
            $percent = 0;
            if ($total_contents && $completed_count) {
                $percent = (int)number_format(($completed_count * 100) / $total_contents);
            ...
            $completed_courses = (array) $user->get_option('completed_courses');
            $completed_courses[$seminar_id]['percent'] = $percent;

            //Save Array Unique
            $content_ids = $completes->toArray();
            $content_ids[] = $content->id;
            $completed_courses[$seminar_id]['content_ids'] = array_unique($content_ids);
...
if ( ! function_exists('seminar_card')) {
    function seminar_card($course, $grid_class = null){
        return view_seminar_template_part('company.template-part.seminar-loop', compact('course', 'grid_class'));
    }
}

if ( ! function_exists('company_course_card')) {
    function company_course_card($course, $grid_class = null){
        return view_seminar_template_part('company.template-part.company-course-loop', compact('course', 'grid_class'));
    }
}
